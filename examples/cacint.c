 ...
 *
 * ----------------------------------------------------------------------
 * Date        : $Date: 2013-09-25 10:49:05 +0200 (Wed, 25 Sep 2013) $
 * Revision    : $Revision: 2499 $
 * Author      : $Author: sp $
 * URL         : $URL: trunk/src/C/cacint.c $
 *
 *  Language    : C
 * ----------------------------------------------------------------------
...
extern "C" {
	...
	//static char *cacint_RevisionID = "$Id: cacint.c 2499 2013-09-25 08:49:05Z sp $";
	...
	*/
	...
	/* Added for ChemApp V550 */

	CMT tqlpar_(LIP INDEXP, CHP OPTION, LIP NOPAR, CHP CHRPAR, LIP LGTPAR, LIP NOERR, ftnlen, ftnlen);
	CMT tqgpar_(LIP INDEXP, CHP OPTION, LIP INDEXX, LIP NOEXPR, LIP NVALA, DBP VALA, LIP NOERR, ftnlen);

	/* Added for ChemApp V634 */

	CMT tqconf_(CHP OPTION, LIP INDEXA, LIP INDEXB, LIP INDEXC, LIP NOERR, ftnlen);
	...
	*/
	...
	CMT TQLPAR(LIP INDEXP, CHP OPTION, LNT OPTIONLEN, LIP NOPAR, CHP CHRPAR, LNT CHRPARLEN, LIP LGTPAR, LIP NOERR);
	CMT TQGPAR(LIP INDEXP, CHP OPTION, LNT OPTIONLEN, LIP INDEXX, LIP NOEXPR, LIP NVALA, DBP VALA, LIP NOERR);


	/* Added for ChemApp V634 */

	CMT TQCONF(CHP OPTION, LNT OPTIONLEN, LIP INDEXA, LIP INDEXB, LIP INDEXC, LIP NOERR);
...
#else
	TQOPEN(FILE, (int)strlen(FILE), &i1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQGIO(OPTION, (int)strlen(OPTION), IVAL, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCIO(OPTION, (int)strlen(OPTION), &i1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQGSU(OPTION, (int)strlen(OPTION), UNIT, TQSTRLEN, NOERR);
#endif
...
#else
	TQCSU(OPTION, (int)strlen(OPTION), UNIT, (int)strlen(UNIT), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQINSC(NAME, (int)strlen(NAME), INDEXS, NOERR);
#endif
	return(*NOERR);
}
...
int tqcsc(CHP NAME, LIP NOERR)
{
	...
	while (*(NAME + i*TQSTRLEN) != 0) {
		strcpy_s(buf + i*(TQSTRLEN - 1), sizeof(buf + i*(TQSTRLEN - 1)), NAME + i*TQSTRLEN);
		for (LI j = (int)strlen(NAME + i*TQSTRLEN); j < TQSTRLEN - 1; j++)
		{
			*(buf + i*(TQSTRLEN - 1) + j) = ' ';
		}
		i++;
...
#else
	TQINP(NAME, (int)strlen(NAME), INDEXP, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQINPC(NAME, (int)strlen(NAME), &i1, INDEXC, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCSP(&i1, OPTION, (int)strlen(OPTION), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCSPC(&i1, &i2, OPTION, (int)strlen(OPTION), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQSETC(OPTION, (int)strlen(OPTION), &i1, &i2, &d1, NUMCON, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQSTTP(IDENTS, (int)strlen(IDENTS), VALS, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQSTCA(IDENTS, (int)strlen(IDENTS), &i1, &i2, &d1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQSTEC(OPTION, (int)strlen(OPTION), &i1, &d1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQSTRM(IDENTS, (int)strlen(IDENTS), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCE(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCEL(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCLIM(OPTION, (int)strlen(OPTION), &d1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQGETR(OPTION, (int)strlen(OPTION), &i1, &i2, VAL, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQGDPC(OPTION, (int)strlen(OPTION), &i1, &i2, VAL, NOERR);
#endif
	return(*NOERR);
}
...
int tqerr(CHP MESS, LIP NOERR)
{
	...
	strcpy_s(MESS, 250, buf);
	strcpy_s(MESS + 80, sizeof MESS + 80, buf + 80);
	strcpy_s(MESS + 160, sizeof MESS + 160, buf + 160);
	return(*NOERR);
}
...
#else
	TQSTXP(IDENTS, (int)strlen(IDENTS), OPTION, (int)strlen(OPTION), VAL, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQMAP(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, ICONT, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQMAPL(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, ICONT, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQOPNA(FILE, (int)strlen(FILE), &i1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQOPNB(FILE, (int)strlen(FILE), &i1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQINLC(NAME, (int)strlen(NAME), &i1, &i2, INDEXC, NOERR);
#endif
	return(*NOERR);
}
...
int tqgnlc(LI INDEXP, LI INDEXL, LI INDEXC, CHP NAME, LIP NOERR)
{
	...
	TQGNLC(&i1, &i2, &i3,(CHP)NAME, TQSTRLEN, NOERR);
#endif
	TQremspaces(NAME, TQSTRLEN);
	return(*NOERR);
}
...
#else
  TQCSU (FILE,(int)strlen(FILE),&i1,FFORM,(int)strlen(FFORM),
	 FSTAT,(int)strlen(FSTAT),FACC,(int)strlen(FACC),&i2,IOSTAT,NOERR);
#endif
  return(*NOERR);
}
...
#else
	TQOPNT(FILE, (int)strlen(FILE), &i1, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQWSTR(OPTION, (int)strlen(OPTION), CTXT, (int)strlen(CTXT), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCEN(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCENL(OPTION, (int)strlen(OPTION), &i1, &i2, VALS, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQWASC(FILE, (int)strlen(FILE), NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQGDAT(&i1, &i2, OPTION, (int)strlen(OPTION), &i3, NVALV, VALV, NOERR);
#endif
	return(*NOERR);
}
...
#else
	TQCNSC(&i1, NAME, (int)strlen(NAME), NOERR);
#endif
	return(*NOERR);
}
...
{
	LI i1 = INDEXP;
	char buf[1999 * 156 + 4000];
#if defined(UNIX) && !defined(CRAY)
	tqlpar_(&i1, OPTION, NOPAR, buf, LGTPAR, NOERR, (ftnlen)strlen(OPTION), 156);
#else
	TQLPAR(&i1, OPTION, (int)strlen(OPTION), NOPAR, buf, 156, LGTPAR, NOERR);
#endif
	...
	for (LI i = 0; i < 1999; i++) {
		buf[((i + 1) * 156) - 1] = 0;
		TQremspaces(buf + (i * 156), 154);
		strcpy_s(CHRPAR + (i * 156), sizeof(CHRPAR + (i * 156)), buf + (i * 156));
	}
...
{
	LI i1 = INDEXP, i2 = INDEXX;

#if defined(UNIX) && !defined(CRAY)
	tqgpar_(&i1, OPTION, &i2, NOEXPR, NVALA, VALA, NOERR, (ftnlen)strlen(OPTION));
#else
	TQGPAR(&i1, OPTION, (int)strlen(OPTION), &i2, NOEXPR, NVALA, VALA, NOERR);
#endif
...
{
	LI i1 = INDEXA, i2 = INDEXB, i3 = INDEXC;
#if defined(UNIX) && !defined(CRAY)
	tqconf_(OPTION, &i1, &i2, &i3, NOERR, (ftnlen)strlen(OPTION));
#else
	TQCONF(OPTION, (int)strlen(OPTION), &i1, &i2, &i3, NOERR);
#endif
	return(*NOERR);
}