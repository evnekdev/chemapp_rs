...
pub enum ChemAppError {
    ...
    CleanupError {
        /// High-level operation whose cleanup also failed.
        operation: String,
        /// Original operation failure.
        primary: Box<ChemAppError>,
        /// Subsequent required-cleanup failure.
    ...
    RetryError {
        /// High-level bounded protocol that exhausted both orderings.
        operation: String,
        /// Failure from the preferred ordering.
        preferred: Box<ChemAppError>,
        /// Failure from the alternate ordering.
        alternate: Box<ChemAppError>,
    },
    /// A non-native failure such as an invalid path, conversion, or library error.
    OtherError(String),
    /// Application-supplied contextual failure retained for compatibility.
    CustomError(String),
}
...
impl std::error::Error for ChemAppError {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            Self::CleanupError { primary, .. } => Some(primary.as_ref()),
            Self::RetryError { preferred, .. } => Some(preferred.as_ref()),
            _ => None,
        }
    }
}
...
lazy_static! {
    /// Known ChemApp native error-code descriptions from the interface reference.
